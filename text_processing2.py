...
def digits_to_words(input_string):
    ...
    * 만약 인풋 스트링에서 숫자가 존재하지 않는 다면, 빈 문자열 (empty string)을 반환함
    ...
    for char in input_string:
        if char.isdecimal():
            digit_string += number_string[int(char)] + " "
...
def to_camel_case(underscore_str):
    ...
    * 만약 어떤 변수 이름이 underscore로만 이루어 진다면, 빈 문자열만 반환해도 됨
    ...
    while (tmp_str.find("__") != -1):
        tmp_str = tmp_str.replace("__", "_")

    i = 0
    while i < len(tmp_str): # 단어 앞의 '_' pass
        if tmp_str[i] == "_":
            i += 1
        else:
            break
    ...
    while i < len(tmp_str):
        if tmp_str[i] == "_":
            if (i + 1 < len(tmp_str)):
                camelcase_str += tmp_str[i + 1].upper()
            i += 2
        else:
            camelcase_str += tmp_str[i]
            i += 1
